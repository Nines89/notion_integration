...
class NObj(ABC):
    ...
    def get_content(self):
        self.data = self._get(api_url=self.get_url, headers=self.header)

    def update_content(self, data: dict):
        self.data = self._update(api_url=self.update_url, data=data)

    @property
    def object_type(self):
        if self.data is None:
            return "Please retrieve object data before calling this method"
    ...
    def object_create_info(self):
        from notion_user import NUser
        if self.data is None:
            return "Please retrieve object data before calling this method"
        return {'create_time': NDate(self.data['created_time']),
                'create_user': NUser(self.header, self.data['created_by']['id'])}
    ...
    def object_last_edit_info(self):
        ...
        return {'last_edited_time': NDate(self.data['created_time']),
                'create_user': NUser(self.header, self.data['last_edited_by']['id'])}
...
class _NObjPatch(_NObjReq):
    def __init__(self, header: dict, url: str, *args, **kwargs):
        super().__init__(header, url, *args, **kwargs)
        self.data = self.update_obj(*args, **kwargs)
